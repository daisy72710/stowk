...
def main():
    ...
    #opening compare buttons
    compare = []
    compare = browser.find_elements_by_xpath("//button[@class='btn btn-sm btn-default btn-block samplePrices']")
    #print(compare)
    browser.execute_script("return arguments[0].scrollIntoView();", compare[0])
    time.sleep(0.5)
    #compare[0].click()
    #browser.execute_script("return arguments[0].scrollIntoView();", compare[10])
    #time.sleep(0.5)
    #compare[10].click()
    ...
    #    print(x)        
        compare[x].click()
        browser.execute_script("return arguments[0].scrollIntoView();", compare[x+1])
    #    print(x+1)
        time.sleep(0.5)
